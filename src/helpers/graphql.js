...
const { print, buildSchema, graphqlSync, introspectionQuery } = require("graphql");
...
class GraphQLHelper {
  ...
  /**
   * Get GraphQL Schema with directives injected
   *
   * @method schema
   *
   * @return {String}
   */
  get schema () {
    let schema = makeExecutableSchema({ typeDefs: this.typeDefs, resolvers: {JSON: GraphQLJSON, ...this.resolvers} });

    this.schemaDirectives.forEach(value => {
  ...
  get typeDefs() {
    ...
    const definitionsPath = `${rootDir}/schema`;
  ...
  schemaStructure(typeDefs) {

    // Convert SDL to Object
    let object = buildSchema(typeDefs);

    // Makes introspection query to schema
    let result = graphqlSync(object, introspectionQuery).data;

    return JSON.parse(JSON.stringify(result.__schema));
  }

  // Get all operation fields and subfield
  fieldsAndSubfield(operation, object = {}) {

    let fields = {};
  
    if(operation.selectionSet && operation.selectionSet.selections) {
      operation.selectionSet.selections.map(selection => {
        object[selection.name.value] = {}
        if(selection.selectionSet){
          fields = this.fieldsAndSubfield(selection, object[selection.name.value]);
        }
        object[selection.name.value] = fields;
      })
    }
   ...
   */
  get resolvers() {
    // Subscription: this.subscriptionsResolvers
    let resolvers = { };
